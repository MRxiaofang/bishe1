	...
	<tr>
		<td width="650" height="585">
				...
				<?php
					...
					$ll="select count(*) as totalRows from mianfei where type='$type' and checkstate=$checkstate order by edate";
					//var_dump($ll);
					$select=mysqli_query($link,$ll);
					$minfo=mysqli_fetch_array($select);
					$totalRows=$minfo[totalRows];
					$sumpage=ceil($totalRows/$pagesize);
					$offet=($page-1)*4;
							
					$tx="select * from mianfei where checkstate=$checkstate and type='$type' limit {$offet},{$pagesize}";
					//var_dump($tx);
				...
				<table width="650" height="540" border="0" cellspacing="0">
													...
													<tr>
														<td>操作：<span style="color:red;"><a href='mianfei_ok.php?id=<?php echo $value['id'];?>&type=<?php echo $type;?>&checkstate=<?php echo $checkstate;?>'>审核</a></span>/<span style="color:red;"><a href='mianfei_del.php?id=<?php echo $value['id'];?>&type=<?php echo $type;?>&checkstate=<?php echo $checkstate;?>'>删除</a><span><hr></td>
													</tr>
													<?php										
															endforeach;	}
												?>
				...
				<table border="0" width="650" height="40">
					<tr>
						<td id="menger" border="0" width="650" height="40">
							<ul id="yema">
								<?php
									for($i = 1;$i<=$sumpage;$i++){
										echo "<li><a href='findfreeinfo.php?page={$i}&checkstate={$checkstate}&type={$type}';>$i</a></li>";
													}
								?>
							</ul>
						</td>
